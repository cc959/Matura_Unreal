...
ATrackingCamera::ATrackingCamera()
{
	...
	image_plate->SetRelativeLocation(FVector(10, 0, 0));
	...
	if (cv_cap.isOpened())
	{
		...
		for (int i = 0; i < zarray_size(detections); i++)
		{
			...
			if (det_tag)
			{
				...
				relative_transformation = FQuat::MakeFromEuler(rotation_matrix.ToQuat().Euler() * FVector(-1, -1, 1)).ToMatrix();
			...
			SetActorRelativeTransform(FTransform(global_transformation));
